...
a.toString=function(){return"[PreloadJS AbstractLoader]"};e.AbstractLoader=c})(window);(function(e){function c(){}var a=function(a){this.initialize(a)},b=a.prototype=new AbstractLoader;a.IMAGE="image";a.SOUND="sound";a.JSON="json";a.JAVASCRIPT="javascript";a.CSS="css";a.XML="xml";a.TEXT="text";a.TIMEOUT_TIME=8E3;b.useXHR=true;b.stopOnError=false;b.maintainScriptOrder=true;b.next=null;b.typeHandlers=null;b.extensionHandlers=null;b._loadStartWasDispatched=false;b._maxConnections=1;b._currentLoads=null;b._loadQueue=null;b._loadedItemsById=null;b._loadedItemsBySrc=null;b._targetProgress=
0;b._numItems=0;b._numItemsLoaded=null;b._scriptOrder=null;b._loadedScripts=null;b.TAG_LOAD_OGGS=true;b.initialize=function(a){this._targetProgress=this._numItemsLoaded=this._numItems=0;this._paused=false;this._currentLoads=[];this._loadQueue=[];this._scriptOrder=[];this._loadedScripts=[];this._loadedItemsById={};this._loadedItemsBySrc={};this.typeHandlers={};this.extensionHandlers={};this._loadStartWasDispatched=false;this.useXHR=a!=false&&e.XMLHttpRequest!=null;this.determineCapabilities()};b.determineCapabilities=
function(){var f=a.lib.BrowserDetect;if(f!=null)a.TAG_LOAD_OGGS=f.isFirefox||f.isOpera};a.isBinary=function(f){switch(f){case a.IMAGE:case a.SOUND:return true;default:return false}};b.installPlugin=function(a){if(!(a==null||a.getPreloadHandlers==null)){a=a.getPreloadHandlers();if(a.types!=null)for(var b=0,d=a.types.length;b<d;b++)this.typeHandlers[a.types[b]]=a.callback;if(a.extensions!=null)for(b=0,d=a.extensions.length;b<d;b++)this.extensionHandlers[a.extensions[b]]=a.callback}};b.setMaxConnections=
function(a){this._maxConnections=a;this._paused||this._loadNext()};b.loadFile=function(a,b){a==null?this._sendError({text:"File is null."}):(this._addItem(a),b!==false&&this.setPaused(false))};b.loadManifest=function(a,b){var d;if(a instanceof Array){if(a.length==0){this._sendError({text:"Manifest is empty."});return}d=a}else{if(a==null){this._sendError({text:"Manifest is null."});return}d=[a]}for(var c=0,e=d.length;c<e;c++)this._addItem(d[c],false);b!==false&&this._loadNext()};b.load=function(){this.setPaused(false)};
b.getResult=function(a){return this._loadedItemsById[a]||this._loadedItemsBySrc[a]};b.setPaused=function(a){(this._paused=a)||this._loadNext()};b.close=function(){for(;this._currentLoads.length;)this._currentLoads.pop().cancel();this._currentLoads=[];this._scriptOrder=[];this._loadedScripts=[]};b._addItem=function(f){f=this._createLoadItem(f);f!=null&&(this._loadQueue.push(f),this._numItems++,this._updateProgress(),f.getItem().type==a.JAVASCRIPT&&(this._scriptOrder.push(f.getItem()),this._loadedScripts.push(null)))};
b._loadNext=function(){if(!this._paused){if(!this._loadStartWasDispatched)this._sendLoadStart(),this._loadStartWasDispatched=true;this._numItems==this._numItemsLoaded&&(this._sendComplete(),this.next&&this.next.load&&this.next.load.apply(this.next));for(;this._loadQueue.length&&this._currentLoads.length<this._maxConnections;){var f=this._loadQueue.shift();f.onProgress=a.proxy(this._handleProgress,this);f.onComplete=a.proxy(this._handleFileComplete,this);f.onError=a.proxy(this._handleFileError,this);
this._currentLoads.push(f);f.load()}}};b._handleFileError=function(a){var a=a.target,b=this._createResultData(a.getItem());this._numItemsLoaded++;this._updateProgress();this._sendError(b);this.stopOnError||(this._removeLoadItem(a),this._loadNext())};b._createResultData=function(a){return{id:a.id,result:null,data:a.data,type:a.type,src:a.src}};b._handleFileComplete=function(b){var g=this,b=b.target,d=b.getItem(),c=this._createResultData(d);this._removeLoadItem(b);c.result=b instanceof a.lib.XHRLoader?
this._createResult(d,c):d.tag;switch(d.type){case a.IMAGE:if(b instanceof a.lib.XHRLoader){c.result.onload=function(){g._handleFileTagComplete(d,c)};return}break;case a.JAVASCRIPT:if(this.maintainScriptOrder){this._loadedScripts[this._scriptOrder.indexOf(d)]=d;this._checkScriptLoadOrder();return}}this._handleFileTagComplete(d,c)};b._checkScriptLoadOrder=function(){for(var a=this._loadedScripts.length,b=0;b<a;b++){var d=this._loadedScripts[b];if(d===null)break;if(d!==true){var d=this.getResult(d.src),
c=this._createResultData(d);c.result=d.result;this._handleFileTagComplete(d,c);this._loadedScripts[b]=true;b--;a--}}};b._handleFileTagComplete=function(a,b){this._numItemsLoaded++;this._loadedItemsById[a.id]=b;this._loadedItemsBySrc[a.src]=b;a.completeHandler&&a.completeHandler(b);this._updateProgress();this._sendFileComplete(b);this._loadNext()};b._removeLoadItem=function(a){for(var b=this._currentLoads.length,d=0;d<b;d++)if(this._currentLoads[d]==a){this._currentLoads.splice(d,1);break}};b._createResult=
function(b,g){var d=null,c;switch(b.type){case a.IMAGE:d=this._createImage();break;case a.SOUND:d=b.tag||this._createAudio();break;case a.CSS:d=this._createLink();break;case a.JAVASCRIPT:d=this._createScript();break;case a.XML:if(e.DOMParser){var h=new DOMParser;h.parseFromString(g,"text/xml")}else h=new ActiveXObject("Microsoft.XMLDOM"),h.async=false,h.loadXML(g),c=h;break;case a.JSON:case a.TEXT:c=g}return d?(b.type==a.CSS?d.href=b.src:d.src=b.src,d):c};b._handleProgress=function(a){var a=a.target,
b=this._createResultData(a.getItem());b.progress=a.progress;this._sendFileProgress(b);this._updateProgress()};b._updateProgress=function(){var a=this._numItemsLoaded/this._numItems,b=this._numItems-this._numItemsLoaded;if(b>0){for(var d=0,c=0,e=this._currentLoads.length;c<e;c++)d+=this._currentLoads[c].progress;a+=d/b*(b/this._numItems)}this._sendProgress({loaded:a,total:1})};b._createLoadItem=function(b){var c={};switch(typeof b){case "string":c.src=b;break;case "object":b instanceof HTMLAudioElement?
(c.tag=b,c.src=c.tag.src,c.type=a.SOUND):c=b}c.ext=this._getNameAfter(c.src,".");if(!c.type)c.type=this.getType(c.ext);if(c.id==null||c.id=="")c.id=c.src;if(b=this.typeHandlers[c.type]||this.extensionHandlers[c.ext]){b=b(c.src,c.type,c.id,c.data);if(b===false)return null;else if(b!==true){if(b.src!=null)c.src=b.src;if(b.id!=null)c.id=b.id;if(b.tag!=null&&b.tag.load instanceof Function)c.tag=b.tag}c.ext=this._getNameAfter(c.src,".")}b=this.useXHR;switch(c.type){case a.JSON:case a.XML:case a.TEXT:b=
true;break;case a.SOUND:c.ext=="ogg"&&a.TAG_LOAD_OGGS&&(b=false)}if(b)return new a.lib.XHRLoader(c);else if(c.tag)return new a.lib.TagLoader(c);else{var d,b="src",e=false;switch(c.type){case a.IMAGE:d=this._createImage();break;case a.SOUND:d=this._createAudio();break;case a.CSS:b="href";e=true;d=this._createLink();break;case a.JAVASCRIPT:e=true,d=this._createScript()}c.tag=d;return new a.lib.TagLoader(c,b,e)}};b.getType=function(b){switch(b){case "jpeg":case "jpg":case "gif":case "png":return a.IMAGE;
case "ogg":case "mp3":case "wav":return a.SOUND;case "json":return a.JSON;case "xml":return a.XML;case "css":return a.CSS;case "js":return a.JAVASCRIPT;default:return a.TEXT}};b._getNameAfter=function(a,b){var c=a.lastIndexOf(b),c=a.substr(c+1),e=c.lastIndexOf(/[\b|\?|\#|\s]/);return e==-1?c:c.substr(0,e)};b._createImage=function(){return document.createElement("img")};b._createAudio=function(){var a=document.createElement("audio");a.autoplay=false;a.type="audio/ogg";return a};b._createScript=function(){var a=
document.createElement("script");a.type="text/javascript";return a};b._createLink=function(){var a=document.createElement("link");a.type="text/css";a.rel="stylesheet";return a};b.toString=function(){return"[PreloadJS]"};a.proxy=function(a,b){return function(c){return a.apply(b,arguments)}};a.lib={};e.PreloadJS=a;c.init=function(){var a=navigator.userAgent;c.isFirefox=a.indexOf("Firefox")>-1;c.isOpera=e.opera!=null;c.isIOS=a.indexOf("iPod")>-1||a.indexOf("iPhone")>-1||a.indexOf("iPad")>-1};c.init();
a.lib.BrowserDetect=c})(window);(function(){var e=function(a,b,c){this.init(a,b,c)},c=e.prototype=new AbstractLoader;c._srcAttr=null;c._loadTimeOutTimeout=null;c.tagCompleteProxy=null;c.init=function(a,b,c){this._item=a;this._srcAttr=b||"src";this._useXHR=c==true;this.isAudio=a.tag instanceof HTMLAudioElement;this.tagCompleteProxy=PreloadJS.proxy(this._handleTagLoad,this)};c.cancel=function(){this._clean();var a=this.getItem();if(a!=null)a.src=null};c.load=function(){this._useXHR?this.loadXHR():this.loadTag()};c.loadXHR=function(){var a=
...
function(){return"[PreloadJS XHRLoader]"};PreloadJS.lib.XHRLoader=c})(window);
