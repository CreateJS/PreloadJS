...
b._loadNext=function(){if(!this._paused){if(!this._loadStartWasDispatched)this._sendLoadStart(),this._loadStartWasDispatched=true;if(this._numItems==this._numItemsLoaded)this.loaded=true,this._sendComplete(),this.next&&this.next.load&&this.next.load.apply(this.next);for(;this._loadQueue.length&&this._currentLoads.length<this._maxConnections;){var f=this._loadQueue.shift();f.onProgress=a.proxy(this._handleProgress,this);f.onComplete=a.proxy(this._handleFileComplete,this);f.onError=a.proxy(this._handleFileError,
this);this._currentLoads.push(f);f.load()}}};b._handleFileError=function(a){var a=a.target,b=this._createResultData(a.getItem());this._numItemsLoaded++;this._updateProgress();this._sendError(b);this.stopOnError||(this._removeLoadItem(a),this._loadNext())};b._createResultData=function(a){var b={id:a.id,result:null,data:a.data,type:a.type,src:a.src};this._loadedItemsById[a.id]=b;return this._loadedItemsBySrc[a.src]=b};b._handleFileComplete=function(b){var b=b.target,g=b.getItem(),d=this._createResultData(g);
this._removeLoadItem(b);d.result=b instanceof a.lib.XHRLoader?this._createResult(g,b.getResult()):g.tag;switch(g.type){case a.IMAGE:if(b instanceof a.lib.XHRLoader){var c=this;d.result.onload=function(){c._handleFileTagComplete(g,d)};return}break;case a.JAVASCRIPT:if(this.maintainScriptOrder){this._loadedScripts[this._scriptOrder.indexOf(g)]=g;this._checkScriptLoadOrder(b);return}}this._handleFileTagComplete(g,d)};b._checkScriptLoadOrder=function(){for(var a=this._loadedScripts.length,b=0;b<a;b++){var d=
this._loadedScripts[b];if(d===null)break;if(d!==true){var c=this.getResult(d.src),d=this.getResult(d.id);d.result=c.result;this._handleFileTagComplete(c,d);this._loadedScripts[b]=true;b--;a--}}};b._handleFileTagComplete=function(a,b){this._numItemsLoaded++;a.completeHandler&&a.completeHandler(b);this._updateProgress();this._sendFileComplete(b);this._loadNext()};b._removeLoadItem=function(a){for(var b=this._currentLoads.length,d=0;d<b;d++)if(this._currentLoads[d]==a){this._currentLoads.splice(d,1);
break}};b._createResult=function(b,g){var d=null,c;switch(b.type){case a.IMAGE:d=this._createImage();break;case a.SOUND:d=b.tag||this._createAudio();break;case a.CSS:d=this._createLink();break;case a.JAVASCRIPT:d=this._createScript();break;case a.XML:e.DOMParser?(c=new DOMParser,c=c.parseFromString(g,"text/xml")):(c=new ActiveXObject("Microsoft.XMLDOM"),c.async=false,c.loadXML(g));break;case a.JSON:case a.TEXT:c=g}return d?(b.type==a.CSS?d.href=b.src:d.src=b.src,d):c};b._handleProgress=function(a){var a=
